...
func getNodeFor(podName string) string {
	url := "http://metis-api-service.kube-system.svc.cluster.local:80/scheduling"
	...
	klog.InfoS("[Scheduler] METIS API Request Body")
	klog.InfoS("    ", string(jsonData))
	...
	if err != nil {
		...
		klog.InfoS("[Scheduler] METIS API Result")
		klog.InfoS("    ", "Pod", podName, "-> Node", targetNode)
...
func (f *frameworkImpl) RunScorePlugins(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodes []*framework.NodeInfo) (ns []framework.NodePluginScores, status *framework.Status) {
	...
	for i, nodePluginScores := range allNodePluginScores {
		...
		klog.InfoS("    - Node", "name", allNodePluginScores[i].Name, "score", allNodePluginScores[i].TotalScore)
	}
